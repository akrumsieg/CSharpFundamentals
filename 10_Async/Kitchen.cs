...
namespace _10_Async
{
    ...
    {
        //async doesn't return objects, it returns tasks
        public async Task<Fries> FryPotatoesAsync(Potato potato)
        {
            if(potato.IsPeeled)
            {
                PrettyPrint("Dropping in the fries", 14);
                await Task.Delay(5000); //await allows other threads to run -- is local to method -- so multiple awaits here are line by line
        ...
        public void PrettyPrint(string step, int color)
        {
            ...
            Console.ForegroundColor = (ConsoleColor)color; //ConsoleColor is enum
